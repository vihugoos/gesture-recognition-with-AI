...
const AddBrowseSearchCards = ({ cards, itemsPerLine }) => {
    ...
    let scale =
        (100 *
            ((carousel_width - btn_width) / itemsPerLine - card_margin * 2)) /
        initial_width;

    let content_titles =
        browseSearchDiv.getElementsByClassName("content-title");
...
const CheckCards = (
    ...
    for (let i = 0; i < carousels.length; i++) {
        ...
        let btn_width =
            carousels[i].getElementsByClassName("carousel-btn")?.[0]
                ?.clientWidth +
                carousels[i].getElementsByClassName("carousel-btn")?.[1]
                    ?.clientWidth ?? 10;
